...
const VideoTransition = ({ isVisible }: { isVisible: boolean }) => {
  const microTraces = useMemo(() => {
    return Array.from({ length: 60 }).map((_, i) => {
      // Basic radiating logic from top-left (0,0)
      ...
      {isVisible && (
        ...
        >
          ...
          <svg className="w-full h-full" viewBox="0 0 1440 900" preserveAspectRatio="xMidYMid slice">
            ...
            {/* Static Base Layer - Dark Metallic Traces */}
            ...
            {/* GPU Animated Spark Layer with Intensified Glow */}
              ...
              {microTraces.map((trace) => (
                <g key={`spark-group-${trace.id}`}>
                  ...
                  <path
                    ...
                    pathLength="100"
                    style={{ animationDelay: `${trace.delay}ms` }}
                    className="spark-animation-orange opacity-60"
                  />
                  {/* Inner Hot Core */}
                  <path
                    d={trace.d}
                    ...
                    pathLength="100"
                    style={{ animationDelay: `${trace.delay}ms` }}
                    className="spark-animation-orange"
                  />
                  {/* Junction Pads */}
                  <circle 
                    cx={trace.targetX} 
                    ...
                    fill="#ff9d00"
                    style={{ 
                      animationDelay: `${trace.delay + 400}ms`,
                      opacity: 0
                    }}
                    className="pad-animation"
                  />
                </g>
              ))}
            </g>
...
const Navbar = ({ setActiveTab }: { setActiveTab: (tab: string) => void }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 overflow-hidden ${isScrolled ? 'bg-background/95 border-b border-primary/20 shadow-md py-2 backdrop-blur-md' : 'bg-transparent py-6'}`}>
      {/* Subtle Video-like scanline for navbar */}
      {isScrolled && (
        <motion.div 
          initial={{ opacity: 0 }}
      ...
      <div className="container flex justify-between items-center relative z-10 transition-all duration-300">
        ...
        <div className="hidden lg:flex items-center gap-10">
            ...
            {navLinks.map((link) => (
              <li key={link.name}>
                <button 
                  ...
                  className="text-[10px] font-ui font-black uppercase tracking-[0.2em] text-muted-foreground hover:text-primary transition-all hover:scale-105"
                >
                  {link.name}
                </button>
              </li>
          ...
          <div className="flex items-center gap-8 border-l border-white/10 pl-8">
            ...
            <Button 
              size="sm" 
              className="relative overflow-hidden font-ui uppercase font-black tracking-widest text-[10px] h-10 px-6 rounded-none bg-primary hover:bg-primary-hover shadow-[0_0_15px_rgba(59,130,246,0.3)] transition-all group" 
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div 
            ...
            className="lg:hidden absolute top-full left-0 w-full bg-secondary/95 border-b border-primary/20 backdrop-blur-xl py-8 px-6 flex flex-col gap-8 overflow-hidden"
          >
            {navLinks.map((link) => (
              <button 
                key={link.name} 
                ...
                className="text-3xl font-display font-black uppercase text-left tracking-tighter"
              >
                {link.name}
              </button>
            ))}
            <div className="flex flex-col gap-6 pt-6 border-t border-white/5">
              <Button className="w-full h-16 uppercase font-ui font-black tracking-[0.2em]" onClick={() => {
                setActiveTab('contact');
                scrollToId('contact');
                setIsMobileMenuOpen(false);
              }}>
...
const Hero = ({ isMobile = false, onRequestService, onTechnicalSolutions }: { isMobile?: boolean, onRequestService?: () => void, onTechnicalSolutions?: () => void }) => {
  ...
  const handleRequestRepair = () => {
    ...
    // Simulate a delay or skip actual navigation if needed, 
    // but the prompt just asks for the loading state visual.
    setTimeout(() => {
  ...
  return (
    <div className="relative w-full flex flex-col justify-center">
      <section className={`relative min-h-[calc(100vh-80px)] w-full transition-all duration-1000 flex items-center ${isMobile ? 'pt-0' : 'pt-24'}`}>
        {/* PCB Trace Overlay */}
        ...
        {!isMobile && (
          <>
            ...
            <motion.div 
              ...
              className="absolute top-0 right-[15%] w-[1px] bg-primary/20 z-[2]"
            />
            ...
            <motion.div 
              ...
              className="absolute bottom-[15%] left-0 h-[1px] bg-primary/20 z-[2]"
            />
            ...
            <motion.div 
              ...
              className="absolute top-[10%] left-[8%] w-[2px] h-[80%] bg-gradient-to-b from-transparent via-primary/30 to-transparent z-[1] origin-top"
            />
          </>
        )}

        {/* Background Intro "Video" (Image with scale animation) */}
        <div className="absolute inset-0 z-0">
          <motion.img 
            ...
            alt="Myanmar Precision Systems Technical Laboratory"
            className="w-full h-full object-cover object-center opacity-60 contrast-110 brightness-95"
          ...
          <motion.div 
            ...
            animate={{ opacity: [0, 0.4, 0.2, 0.5, 0.2], scaleX: [0, 1.2, 1, 1.5, 1], x: ['-5%', '5%', '0%'] }}
            transition={{ duration: 5, delay: 1, repeat: Infinity, repeatDelay: 4 }}
            className="anamorphic-flare top-1/4 left-0 h-[1px] bg-primary/40 shadow-[0_0_15px_rgba(59,130,246,0.4)]" 
          />
        </div>
        ...
        <div className={`container relative z-10 flex flex-col items-start text-left ${isMobile ? 'px-4' : ''}`}>
          <motion.div 
            ...
            whileInView="visible"
            viewport={{ once: true }}
            className={`${isMobile ? 'max-w-full p-6' : 'max-w-3xl p-6 md:p-8'} relative group isolate overflow-hidden`}
          >
            {/* HUD element background - subtle */}
            ...
            {/* Custom Yellow Accent Line from theme */}
            <motion.div 
              initial={{ width: 0 }}
              whileInView={{ width: '100%' }}
              transition={{ duration: 1, delay: 0.8 }}
              className="h-1 bg-[#FBBF24] mb-6 max-w-[120px] sm:max-w-sm shadow-[0_0_15px_rgba(251,191,36,0.6)]" 
            />
            ...
            <motion.div variants={itemVariants} className={`flex flex-col sm:flex-row gap-4 ${isMobile ? 'mb-10' : ''}`}>
              <Button 
                size="lg" 
                className="px-6 h-11 font-ui uppercase font-black text-[9px] tracking-[0.4em] shadow-lg shadow-primary/20 transition-all hover:scale-105 active:scale-95 group relative overflow-hidden" 
              ...
              >
                ...
                {isRequestingRepair ? (
                  <>
                    Linking... <RefreshCw size={12} className="ml-2 animate-spin" />
                  </>
                ) : (
                  <>
                    REQUEST SERVICE <ArrowRight size={12} className="ml-2 group-hover:translate-x-2 transition-transform" />
                  </>
                )}
              </Button>
            ...
            {/* Scanline Sweep inside Card - Faster and Flashier - Toned down slightly */}
            <motion.div 
              animate={{ top: ['-100%', '200%'] }}
              transition={{ duration: 3, repeat: Infinity, ease: "linear" }}
              className="absolute left-0 w-full h-[1.5px] bg-gradient-to-r from-transparent via-primary/50 to-transparent opacity-40 z-20 pointer-events-none"
            />
...
const TechnicalMatrix = ({ isMobile = false, onContactClick }: { isMobile?: boolean, onContactClick?: () => void }) => {
  ...
  const laboratoryImages = [
    {
      url: "Survey instruments.jpeg",
      title: "SURVEY CALIBRATION",
      tag: "PRECISION",
      desc: "Total Stations & Theodolites",
      features: [
        "Angle accuracy verification",
        "EDM distance calibration",
    ...
    },
    {
      ...
      tag: "GEODETIC",
      desc: "Satellite Positioning Units",
      features: [
        "Antenna phase center check",
        "Receiver noise analysis",
        "Network connection test",
        "Signal tracking optimization"
      ]
    },
    {
      url: "Compression machine.jpg",
      title: "COMPRESSION TESTING",
      tag: "NDT TESTING",
      desc: "Universal Testing Machines",
      features: [
        "Load cell verification",
        "MCU board revival & repair",
        "Piston speed monitoring",
        "Safety system audit"
      ]
    },
    {
      ...
      tag: "DEVELOPMENT",
      desc: "MCU Systems & Motor Control",
      features: [
        "Custom MCU integration",
    ...
    {
      url: "Construction Lab Equipment.jpg",
      title: "LAB EQUIPMENT",
      tag: "ANALYSIS",
      desc: "Testing & Measurement Tools",
      features: [
        "Balance & scale precision",
        "Oven temp uniformity",
        "Sieve mesh integrity",
        "Sensor validation"
      ]
    }
  ];
  ...
  return (
    <section id="services" className={`relative overflow-hidden ${isMobile ? 'py-4' : 'py-8'} bg-[#020408]`}>
      {/* Background elements: Grid and Ambient Glows */}
      ...
      <div className="container relative z-10 px-6 sm:px-12">
        <div className={`${isMobile ? 'flex flex-col' : 'grid lg:grid-cols-[0.5fr_2fr] gap-12 items-start'}`}>
          ...
          <motion.div 
             ...
             viewport={{ once: true }}
             className={`${isMobile ? 'mb-4 text-center' : 'sticky top-24'}`}
          >
             ...
             <h2 className={`${isMobile ? 'text-2xl' : 'text-4xl'} font-black text-white mb-1 md:mb-2 tracking-tighter leading-none uppercase`}>
                TECHNICAL <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-blue-400 to-primary">SOLUTIONS</span>
             </h2>
             ...
             <div className={`${isMobile ? 'flex flex-row items-center gap-3 mb-4 mt-2' : 'space-y-6'} ${isMobile ? 'mx-auto justify-center' : ''}`}>
                ...
                {!isMobile && (
                  <div className="relative group">
                      ...
                      <p className="text-[13px] text-white/40 font-medium leading-relaxed pl-6">
                        Professional calibration for precision instruments. 
                      </p>
                  </div>
                )}
                
                {isMobile && (
                  <div className="px-2 py-1 bg-white/[0.02] border border-white/5 flex flex-col justify-center items-center h-fit">
                    <ShieldCheck size={12} className="text-primary mb-1" />
                    <span className="text-[8px] font-mono font-bold text-white/20 uppercase">TRUST</span>
                  </div>
                )}
             </div>
             ...
             <div className={`${isMobile ? 'hidden' : 'space-y-4'}`}>
                  <Button 
                    className={`${isMobile ? 'h-11 py-2' : 'h-12'} w-full uppercase font-black tracking-[0.3em] text-[10px] rounded-none bg-primary hover:bg-primary/90 text-black shadow-[0_5px_15px_rgba(var(--primary),0.2)] transition-all group/cta overflow-hidden relative`}
                    onClick={() => onContactClick ? onContactClick() : scrollToId('contact')}
                  >
                    <span className="relative z-10 flex items-center gap-3">
                      REQUEST SERVICE <ArrowRight size={14} className="group-hover/cta:translate-x-2 transition-transform" />
                    </span>
                  </Button>
          ...
          {/* Image Grid: Right Side - Premium Technical Cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6">
             {laboratoryImages.map((img, i) => (
                <motion.div 
                  ...
                  viewport={{ once: true }}
                  className={`group relative ${isMobile ? 'h-[320px]' : (i === 0 ? 'md:h-[720px] md:row-span-2' : 'md:h-[350px]')} overflow-hidden bg-black/60 border border-white/5 hover:border-primary/40 transition-all duration-700 shadow-2xl relative`}
                >
                  <Dialog open={openDialogIndex === i} onOpenChange={(open) => setOpenDialogIndex(open ? i : null)}>
                    <DialogTrigger className="w-full h-full text-left p-0 border-0 bg-transparent cursor-pointer block relative group/card">
                      <div className="w-full h-full overflow-hidden relative bg-[#05070a]">
                        ...
                        <img 
                          src={img.url} 
                          className={`w-full h-full object-contain ${i === 0 && !isMobile ? 'p-10' : 'p-4'} opacity-100 group-hover:scale-105 transition-all duration-1000 ease-out z-10 relative`} 
                          alt={img.title} 
                        />
                        
                        {/* Dynamic Glow and Gradient Overlay */}
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 z-30 bg-gradient-to-t from-black/80 via-black/40 to-transparent">
                          <div className="flex flex-col gap-2">
                              <span className="inline-flex w-fit px-3 py-1 bg-primary text-black text-[8px] font-black uppercase tracking-[0.2em] rounded-none">
                                {img.tag}
                              </span>
                    ...
                    <DialogContent className="sm:max-w-[1100px] w-[98vw] bg-[#020408] border-primary/20 p-0 overflow-hidden text-foreground backdrop-blur-3xl">
                        ...
                        <div className="md:w-[30%] p-6 md:p-10 flex flex-col justify-between bg-gradient-to-br from-black to-[#05070a]">
                           <div className="space-y-6">
                             <div className="flex items-center gap-4">
                                <span className="text-[10px] font-mono font-black text-primary uppercase tracking-[0.4em]">{img.tag}</span>
                           ...
                           <div className="space-y-2 mt-8">
                             <Button 
                               className="w-full h-12 uppercase font-black tracking-[0.3em] text-[10px] rounded-none bg-primary hover:bg-primary/90 text-black transition-all active:scale-95" 
                               onClick={() => {
                             ...
                             <Button 
                               variant="ghost"
                               className="w-full h-10 text-[9px] font-black text-white/20 hover:text-white/40 uppercase tracking-widest rounded-none"
...
const EquipmentBento = ({ isMobile = false, onContactClick }: { isMobile?: boolean, onContactClick?: () => void }) => {
    ...
    const items = [
        { 
          ...
          cols: 'md:col-span-2', 
          desc: 'High-precision angular and distance measurement units for infrastructure development.',
          specs: 'ACCURACY: 0.5" - 2"',
          features: ['Laser plummet', 'Dual-axis compensation', 'Reflectorless EDM up to 1000m', 'Auto-Correction', '8K Data Log']
        },
        { 
          name: 'AUTO LEVELS', 
          img: 'https://images.unsplash.com/photo-1504307651254-35680f356dfd?auto=format&fit=crop&q=80&w=800', 
          cols: 'md:col-span-1', 
          desc: 'Automatic and digital levelling for site preparation and precise height transfer.',
          specs: '±0.3mm to 1.5mm',
          features: ['Air-damped compensator', 'Waterproof IPX6', '32x Magnification', 'Precision Circle']
        },
        { 
          name: 'COMPRESSION UNITS', 
          img: 'https://images.unsplash.com/photo-1581092160562-40aa08e78837?auto=format&fit=crop&q=80&w=1200', 
          cols: 'md:col-span-3', 
          desc: 'High-power testing systems. We provide deep-level technical recovery, reviving failed control boards with custom MCU solutions and DC motor controllers.',
          specs: 'CAPACITY UP TO 3000kN',
          features: ['Automatic pace control', 'MCU-based revival', 'DC Motor Controlling', 'Heavy Duty Frame']
        },
    ];
            ...
            <div className="container">
                <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-8">
                    ...
                    <p className="text-muted-foreground font-medium max-w-sm text-right">
                       Official servicing for premium geotechnical and mechanical assets. Click to inspect specifications.
                    </p>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-4">
                    ...
                    {items.map((item, i) => (
                        <div key={item.name} className={item.cols}>
                          <Dialog open={openDialogIndex === i} onOpenChange={(open) => setOpenDialogIndex(open ? i : null)}>
                            <DialogTrigger className="relative group aspect-video md:aspect-[16/6] overflow-hidden border border-primary/30 rounded-sm cursor-pointer block w-full p-0 text-left bg-transparent shadow-lg shadow-primary/5 transition-all hover:border-primary/50">
                              <motion.div 
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-background via-transparent to-transparent flex flex-col justify-end p-6 md:p-10">
                                    <div className="flex justify-between items-end gap-4">
                                        <div className="max-w-md text-left uppercase">
                                            ...
                                            <p className="text-xs text-muted-foreground normal-case font-medium max-w-sm line-clamp-2 transition-all duration-500">
                                              {item.desc}
                                            </p>
                                        </div>
                                        <div className="hud-corner bottom-0 right-0 p-4 bg-primary text-primary-foreground scale-0 group-hover:scale-100 transition-transform shadow-lg shadow-primary/20">
                                           <PlusIcon size={20} />
                                        </div>
                                    </div>
                                </div>
                          ...
                          <DialogContent className="sm:max-w-[700px] bg-background border-primary/10 p-0 overflow-hidden text-foreground">
                                ...
                                <div className="p-10 flex flex-col">
                                   ...
                                   <div className="space-y-4 mb-10">
                                      ...
                                      <ul className="space-y-2">
                                         {item.features.map(f => (
                                           <li key={f} className="text-xs font-semibold flex items-center gap-2">
                                   ...
                                   <Button className="mt-auto w-full h-14 uppercase font-black tracking-widest text-[10px]" onClick={() => {
                                     if (onContactClick) onContactClick();
                                     scrollToId('contact');
                                     setOpenDialogIndex(null);
                                   }}>
            ...
            <div className="space-y-6 mb-12">
               <div className="flex items-start gap-6 border-l-2 border-primary pl-10">
                  <div>
                    ...
                    <p className={`${isMobile ? 'text-sm' : 'text-lg'} font-bold text-foreground leading-relaxed`}>
               ...
               <div className="pl-10">
                  <a 
                    href="https://wa.me/959428014092" 
                    target="_blank" 
                    rel="noreferrer"
                    className="inline-flex items-center gap-3 px-6 py-4 bg-[#25D366] text-white font-black uppercase text-[10px] tracking-[0.2em] shadow-lg shadow-green-500/20 hover:scale-105 active:scale-95 transition-all"
                  >
                    <Phone size={16} />
        ...
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
               ...
               >
                  ...
                  <Input required className="h-10 bg-secondary/30 border-primary/30 rounded-none focus:border-primary backdrop-blur-sm pr-1 transition-all" placeholder="NAME" />
               </motion.div>
               <motion.div 
                 className="space-y-2"
                 whileFocus={{ scale: 1.02 }}
               ...
               >
                  ...
                  <Input required className="h-10 bg-secondary/30 border-primary/30 rounded-none focus:border-primary backdrop-blur-sm pr-1 transition-all" placeholder="COMPANY" />
               </motion.div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
               ...
               >
                  ...
                  <Input required type="tel" className="h-10 bg-secondary/30 border-primary/30 rounded-none focus:border-primary backdrop-blur-sm pr-1 transition-all" placeholder="+95" />
               </motion.div>
               <motion.div 
                 className="space-y-2"
                 whileFocus={{ scale: 1.02 }}
               ...
               >
                  ...
                  <Input required type="email" className="h-10 bg-secondary/30 border-primary/30 rounded-none focus:border-primary backdrop-blur-sm pr-1 transition-all" placeholder="EMAIL" />
               </motion.div>
            </div>
            ...
            >
                ...
                <Input required className="h-10 bg-secondary/30 border-primary/30 rounded-none focus:border-primary backdrop-blur-sm pr-1 transition-all" placeholder="BRAND / MODEL" />
            </motion.div>
            ...
            >
                ...
                <Textarea required className="h-20 bg-secondary/30 border-primary/30 rounded-none focus:border-primary p-4 resize-none backdrop-blur-sm pr-1 transition-all" placeholder="DESCRIBE THE ISSUE" />
            </motion.div>
            ...
            <Button 
              ...
              className={`relative overflow-hidden w-full h-14 text-[10px] font-black uppercase tracking-[0.4em] shadow-[0_0_30px_rgba(68,157,251,0.2)] hover:scale-[1.02] active:scale-95 transition-all duration-500 rounded-none ${status === 'complete' ? 'bg-green-600' : 'bg-primary'}`}
            >
                <AnimatePresence mode="wait">
                  {status === 'idle' && (
                    <motion.div
                  ...
                  {status === 'complete' && (
                    ...
                    >
                      <ShieldCheck size={20} />
                      <span>MISSION_SUCCESS_SENT</span>
                    </motion.div>
                  )}
                </AnimatePresence>
            </Button>
...
const TechnicalSolutions = ({ isMobile = false, onContactClick }: { isMobile?: boolean, onContactClick?: () => void }) => {
  return (
     <section id="solutions" className={`relative overflow-hidden ${isMobile ? 'h-full py-1 flex flex-col overflow-y-auto pb-6 custom-scrollbar pr-1' : 'py-10'} bg-[#020408]`}>
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <motion.div 
           ...
           className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-blue-900/10 blur-[150px] rounded-full" 
        />
      </div>
      ...
      <div className={`${isMobile ? 'px-4' : 'container px-6 sm:px-12'} relative z-10`}>
        <div className={`${isMobile ? 'flex flex-col' : 'grid lg:grid-cols-2'} ${isMobile ? 'gap-4' : 'gap-12 lg:gap-16'} items-center`}>
          <motion.div
            ...
            viewport={{ once: true }}
            className={`relative ${isMobile ? 'text-center mb-4' : ''}`}
          >
            {/* PCB Annotations */}
            ...
            {!isMobile && (
              <motion.div 
                ...
                transition={{ duration: 1, delay: 0.5 }}
                className="absolute top-1/2 -left-12 w-12 h-[1px] bg-primary/40 z-0 origin-right"
              />
            )}
            ...
            <h2 className={`${isMobile ? 'text-2xl' : 'text-3xl md:text-5xl'} font-black text-white mb-2 md:mb-6 tracking-tighter leading-none uppercase`}>
              EMBEDDED <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-400">SOLUTIONS</span>
            </h2>
            ...
            <div className={`space-y-2 mb-4 ${isMobile ? 'text-left' : 'mb-8 text-left'}`}>
              <div className={`${isMobile ? 'p-1.5 inline-block' : 'p-6'} bg-white/[0.02] border-l border-primary/40 border-y border-r border-white/5 backdrop-blur-sm relative overflow-hidden`}>
                ...
                {isMobile && <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                  <div className="w-1 h-1 rounded-full bg-blue-400 animate-pulse" />
                  <span className="text-[6px] font-black text-blue-400 uppercase">BETA</span>
                </div>}
              </div>
              
              <div className="space-y-1">
                <p className={`${isMobile ? 'text-[10px]' : 'text-sm'} text-white/40 font-medium leading-tight uppercase`}>
                  Reviving legacy machinery via custom MCU architectures. Specialized in Compression Machine board replacement and DC Motor control systems.
                </p>
              ...
              <div className={`grid ${isMobile ? 'grid-cols-4' : 'grid-cols-2'} gap-1 md:gap-2 mt-4`}>
                {[
                  { title: "MCU DEV", desc: "Custom Control" },
                  { title: "DC MOTOR", desc: "Control Logic" },
                  { title: "BOARD REVIVE", desc: "PCB Recovery" },
                  { title: "IN DEV", desc: "Future Support" }
                ].map((item, idx) => (
            ...
            <Button 
              className={`h-11 uppercase font-black tracking-[0.3em] text-[10px] rounded-none bg-transparent border border-primary/30 text-primary hover:bg-primary hover:text-black transition-all group ${isMobile ? 'w-full' : 'px-10'}`}
              onClick={() => onContactClick ? onContactClick() : scrollToId('contact')}
            >
          ...
          >
            ...
            <div className="relative aspect-square sm:aspect-video lg:aspect-square overflow-hidden bg-[#05070a] border border-white/10 p-4">
              ...
              <img 
                src="images (6).jpeg" 
                alt="Technical Maintenance Solutions" 
...
export default function App() {
  ...
  const handleTabChange = (tab: string) => {
    ...
    // Switch tab at peak of radiating surge (~800ms)
    setTimeout(() => {
      setActiveTab(tab);
      setTimeout(() => scrollToId(tab), 50);
    }, 800);
    
    // Complete transition state - matching ~1.3s total window for smoothest tail-off
    setTimeout(() => {
        ...
        case 'home':
          return (
            <div className="min-h-full" id="home">
              <Hero 
                onRequestService={() => handleTabChange('contact')} 
                onTechnicalSolutions={() => handleTabChange('services')} 
              />
            </div>
          );
        case 'services':
  ...
  return (
    ...
    >
        ...
        {showLetterbox && (
          <>
            <motion.div 
              ...
              className="fixed left-0 w-full h-[12vh] bg-primary/10 z-[2000] pointer-events-none border-b border-primary/10" 
            />
            <motion.div 
              initial={{ bottom: 0 }}
              animate={{ bottom: '-15%' }}
              exit={{ bottom: '-20%' }}
              transition={{ duration: 1.2, delay: 1.2, ease: [0.22, 1, 0.36, 1] }}
              className="fixed left-0 w-full h-[12vh] bg-primary/10 z-[2000] pointer-events-none border-t border-primary/10" 
            />
          </>
        )}
      </AnimatePresence>
      ...
      {/* Electronic Theme Additions - Subtle */}
      ...
      {/* Main Tabbed Content Area with Integrated Fade Page Transition */}
      <main className="min-h-screen pt-20 pb-16 lg:pb-0 relative overflow-hidden">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-secondary/90 border-t border-primary/20 backdrop-blur-lg px-2 py-4 flex justify-around items-center">
        <button 
          ...
          className={`flex flex-col items-center gap-1 transition-all duration-300 ${activeTab === 'home' ? 'text-primary scale-110' : 'text-muted-foreground opacity-60'}`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center gap-1 transition-all duration-300 ${activeTab === 'services' ? 'text-primary scale-110' : 'text-muted-foreground opacity-60'}`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center gap-1 transition-all duration-300 ${activeTab === 'solutions' ? 'text-primary scale-110' : 'text-muted-foreground opacity-60'}`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center gap-1 transition-all duration-300 ${activeTab === 'contact' ? 'text-primary scale-110' : 'text-muted-foreground opacity-60'}`}
        >
      ...
      {!isMobile && (
        <div className="fixed bottom-0 left-0 right-0 z-50 h-10 bg-black/40 backdrop-blur-md border-t border-white/5 flex items-center justify-between px-10 pointer-events-none">
          <div className="flex gap-12">
            <div className="flex items-center gap-2">
              <div className="w-1.5 h-1.5 bg-green-500 animate-pulse rounded-full pcb-pulse" />
              <span className="pcb-label">CORE_LINK: ACTIVE</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-1.5 h-1.5 bg-blue-500 animate-pulse rounded-full delay-500" />
              <span className="pcb-label font-bold text-blue-400/80">TX: ...STREAMING</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="pcb-label text-white/20">BUFFER:</span>
              <span className="pcb-label">1024KB</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="pcb-label text-white/20">FREQ:</span>
              <span className="pcb-label">168MHz</span>
            </div>
          </div>
      ...
      <style>{`
        ...
        .text-shadow-glow {
          text-shadow: 0 0 10px rgba(0, 180, 255, 0.6), 0 0 20px rgba(0, 180, 255, 0.2);
        }
        .hero-glow {
          background: radial-gradient(circle at var(--mouse-x, 50%) var(--mouse-y, 50%), rgba(0, 180, 255, 0.15) 0%, transparent 60%);
        }
        .h-screen-nav {
